...
.thumbnail:hover {background-color: #ddd;}
    ...
    <div class="jumbotron" style="background-color: #4CAF50">
         ...
         <div class="row">
        <?php
          $select=mysqli_query($koneksi, "SELECT * FROM materi_sedang order by Id_materi asc");
        ...
        .thumbnails{
          ...
          <div class="col-sm-3 col-md-3">
            <div class="thumbnail thumbnails">
              <img src="img/sedang/materi<?php echo $isi['Id_materi'] ?>.jpg"width="100%" height="250">
                <div class="caption">
                  <h3><?php echo $isi['judul_materi'] ?></h3>
                  <p><a href="index.php?page=materi<?php echo $isi['Id_materi']+5 ?>" class="btn btn-primary" role="button">Lihat</a> </p>
                </div>
            </div>
          </div>
        <?php } ?> 
...
<!-- Latihan Soal -->
    ...
    if($Id_User){
      ...
      if($isi['score_lat_mudah']>=75){
    ?>
    <div align="center">
      <div class="jumbotron" style="width: 50%; margin-top: 50px;" align="center">
        <h1 class="display-3">Hello, <?php echo "$User_Name"; ?></h1>
        <p class="lead">Setelah mempelajari materi di tingkat sedang, Silahkan uji kemampuan anda dalam bahasa indramayu dengan mengerjakan latihan soal yang kami sediakan berdasarkan pelajaran/materi yang ada di materi sedang, Good Luck!</p>
        <hr class="my-4">
        <?php
          if($isi['score_lat_sedang']>=1){
        ?>
        <p><h6 class="display-5">Score terahir Anda mengerjakan Latihan soal : <?php echo $isi['score_lat_sedang'] ?></p>
        <?php
          }
          else{
        ...
        ?>
        <p class="lead">
          <a class="btn btn-primary btn-lg" href="index_lat_sedang.php" role="button">Mulai</a>
        </p>
      </div>
    </div>
    <?php
      }
    else if($isi['score_lat_sedang']<75){
      echo "<div class='alert alert-info'>
            <strong>Info!</strong> Ayo kerjakan latihan soal di tingkat mudah dan raihlah score minimal 75 untuk dapat membuka latihan soal pada tingkat sedang.
          </div>";
    }
  }
    else {
